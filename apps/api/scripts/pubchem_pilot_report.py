...
`build_pilot_report()` é a função reutilizável de verdade (extraída nesta correção -- Run 2,
`INVALID_FALSE_POSITIVE` -- para ser compartilhada com `pubchem_pilot_wait_for_terminal.py` e
`pubchem_pilot_check_idempotency.py` sem duplicar a lógica de consulta nem reabrir uma sessão
por script auxiliar)."""
...
def build_pilot_report(db: Session, request_id: str) -> dict | None:
    """Constrói o relatório JSON de uma solicitação de ingestão, ou `None` se `request_id` não
    existir. Sempre consulta o banco DE NOVO (nunca reaproveita um objeto ORM já carregado por
    outra parte do processo) -- quem chama é responsável por, se necessário, ter chamado
    `db.expire_all()`/usar uma sessão nova antes, para nunca servir estado obsoleto."""
    request = db.get(ScientificIngestionRequest, request_id)
    if request is None:
        return None
...
def main() -> int:
    ...
    db = SessionLocal()
    try:
        report = build_pilot_report(db, args.request_id)
        if report is None:
