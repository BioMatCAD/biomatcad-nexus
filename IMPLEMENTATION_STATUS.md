...
| 14 | E2E real | Escrito, **nunca executado em nenhum ambiente (navegador)** | `apps/web/e2e/` (Playwright) -- reconfirmado bloqueado neste sandbox Linux em 2026-07-29 por dupla causa: `libXdamage.so.1` ausente (Chromium E Firefox, mesma classe de dependência) e rede bloqueada para obter a biblioteca sem `sudo` (`apt-get download` -> 502 Bad Gateway) -- ver `apps/web/e2e/README.md` e `TEST_EVIDENCE.md` §8. Backend (83/2/0) e frontend (tsc/eslint/27 vitest/build) revalidados sem regressão isoladamente nesta mesma data, mas isso não substitui o percurso E2E via navegador; pendente de execução real no Windows do usuário. |
