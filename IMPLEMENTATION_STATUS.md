...
| 14 | E2E real | Escrito, **executado pela primeira vez no Windows do usuário -- global-setup falhava com bug real, agora corrigido** | Usuário rodou `npm run test:e2e` de verdade após instalar o Chromium; `global-setup.ts` falhava com `ReferenceError: __dirname is not defined` (módulo ES não tem `__dirname`) antes de qualquer teste. Corrigido: diretório do módulo via `fileURLToPath(import.meta.url)`, seleção de Python multiplataforma (`python` no Windows, `python3` nas demais), ambos extraídos em funções testáveis com 8 testes de regressão em `apps/web/tests/e2eGlobalSetup.test.ts` (guarda verificada deliberadamente quebrando e restaurando a correção). Reexecução real pós-correção neste sandbox confirma o global-setup concluindo sem erro; a suíte segue bloqueada apenas pela ausência de `libXdamage.so.1` (Chromium/Firefox) neste sandbox -- ver `apps/web/e2e/README.md` e `TEST_EVIDENCE.md` §8-9. Pendente: execução real completa (com backend/API/dispatcher reais) no Windows do usuário, agora sem o bloqueio do `__dirname`. |
