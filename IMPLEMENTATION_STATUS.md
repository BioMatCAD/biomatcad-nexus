...
| 11 | Manifesto coerente com os artefatos | Código corrigido (a); gate automatizado criado e validado por dry-run real (b); **execução real com PicoGK ainda pendente** (c) | Manifesto reestruturado, verificado por testes de unidade/integração do backend. Gate `apps/api/scripts/verify_full_pipeline_sha256.py` criado: submete job NOVO via HTTP real (nunca pré-semeado), invoca o dispatcher real, compara SHA-256 em 5 fontes (físico/Artifact via API/Artifact via DB/Manifest/download), confirma métricas/auditoria/associação. Dry-run real neste sandbox (Postgres+API reais via pgserver/uvicorn) confirmou toda a mecânica HTTP e a transição queued→running (via started_at) funcionando corretamente; o job falhou honestamente com `WORKER_RUNTIME_UNAVAILABLE` (esperado, sem PicoGK linux-x64) -- nenhum sucesso fabricado. Falta a execução real no Windows do usuário (kit em `docs/examples/WINDOWS_EXECUTION_KIT.md` §9) para fechar este item de fato. |
...
| 18 | Iniciador executável (launcher) do BioMatCAD Nexus para Windows | **Código completo, testado (54/54 xUnit reais) e cross-publicado; execução real do `.exe` no Windows do usuário ainda pendente** | `tools/windows-launcher/` (C#/.NET 9): implementa os 16 comportamentos pedidos (localizar raiz do repo, detectar Python/Node/npm/.NET, criar `.venv`, `pip install -e .` condicional, checar `node_modules`, gerar `API_SECRET_KEY` efêmero via CSPRNG, `ENVIRONMENT=test`, iniciar API/frontend, aguardar prontidão via socket TCP real, abrir `http://localhost:5173/login`, mostrar status/PIDs, encerrar só os filhos rastreados, detectar portas ocupadas, nunca expor o segredo, banner permanente "AMBIENTE DE TESTE"). 54 testes xUnit **reais** (sockets TCP reais, subprocessos reais, diretórios temporários reais — sem fakes de SO) cobrindo os 10 cenários pedidos, incluindo o teste explícito de Python 3.14 (comparação de versão numérica, não lexicográfica) e testes de guarda de segurança (ArgumentList em vez de Arguments concatenado, `UseShellExecute=true` só na URL fixa do navegador, sem `taskkill`/Docker/sudo/admin, segredo nunca logado). `dotnet publish -r win-x64 --self-contained false -p:PublishSingleFile=true` **cross-publicado com sucesso** neste sandbox Linux, gerando um `.exe` PE32+ Windows real (confirmado via `file`), com SHA-256 calculado — mas **apenas compilado, nunca executado**, pois este sandbox não roda binários win-x64. Falta o usuário rodar `Start-BioMatCAD.cmd`/o `.exe` no Windows real e confirmar os 16 comportamentos ponta-a-ponta (item explicitamente reservado para a próxima rodada de evidência do usuário, mesmo padrão já usado para o E2E Playwright no item 14). |
