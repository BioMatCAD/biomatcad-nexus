...
| 14 | E2E real | Escrito, **executado de verdade no Windows do usuário duas vezes -- dois bugs reais encontrados e corrigidos** | 1ª rodada: bug de bootstrap (`ReferenceError: __dirname`), corrigido. 2ª rodada (após o fix): os dois testes RODARAM mas falharam -- causa raiz era o próprio teste usando `page.goto()` para rotas protegidas (derruba o token em memória via reload, ver AuthContext.tsx) e afirmando o literal `"succeeded"` que a UI nunca renderiza (mostra "Concluído"). Corrigido: navegação 100% via clique em UI real (`getByRole`), verificação de status via `data-testid` documentado. Guardas de regressão verificadas de verdade (quebra deliberada + restauração) em `apps/web/tests/verticalSpecGuard.test.ts` e `apps/web/tests/JobDetailPage.test.tsx`. Ver `apps/web/e2e/README.md` e `TEST_EVIDENCE.md` §10. Reexecução real neste sandbox confirma o bloqueio de navegador (`libXdamage.so.1`) como única causa restante -- não mais os bugs de navegação/asserção. Pendente: nova execução real completa no Windows do usuário para confirmar a correção contra a interface de produção. |
| 15 | Todos os testes/builds aprovados | **SIM** | Backend: 83 testes pytest (2 skips esperados sem `dotnet`/Windows) revalidados em 2026-07-29 contra PostgreSQL real via `pgserver`, 0 falhas; ruff e mypy limpos. Worker C#: 62/62 xUnit passando neste sandbox E build Release real aprovado no Windows do usuário (não só neste sandbox). Frontend: 27/27 Vitest, `tsc --noEmit` e `eslint` limpos, `npm run build` executado com sucesso -- todos revalidados em 2026-07-29. Falta ainda: E2E Playwright real via navegador (item 14, bloqueado neste sandbox). |
